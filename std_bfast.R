...
path <- "~/Desktop/Python/IC/scripts/IC/.ipynb_checkpoints/Cods.Mira/"
setwd(path)

files_median <- list.files(path = path,pattern = '*std*')
files_std <- list.files(path = path,pattern = '*median*')

df_median <- lapply(files_median,function(x){read_csv(x) %>% filter(lubridate::year(Timestamp) %in% c(2008:2021))})
df_std <- lapply(files_std,function(x){return(read_csv(x) %>%
                                                select(c(NDVI,Timestamp)) %>%
                                                filter(lubridate::year(Timestamp) %in% c(2008:2021)))})

ts_median <- lapply(df_median, function(x){bfastts(data = x$NDVI,dates = x$Timestamp, type = c("irregular"))})
